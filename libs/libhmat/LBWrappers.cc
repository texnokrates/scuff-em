...
{
  if (StorageType!=LHM_NORMAL)
   ErrExit("SVD() not supported for packed-storage matrices");

  /***************************************************************/
  /***************************************************************/
  /***************************************************************/
  int NRCMin = (NR<NC ? NR : NC);
  if ( Sigma && ( (Sigma->N!=NRCMin) ) )
   { Warn("Incorrect Sigma vector passed to SVD (reallocating)");
     delete Sigma;
     Sigma=0;
   };
  if (Sigma==0)
